...
def fabriquer_qr_lune_souverain():
    # 1. Configuration du QR Code à Tolérance Maximale H (30% de perte acceptée)
    qr = qrcode.QRCode(
        version=5,
    ...
    # 2. Injection du lien universel capturé par l'App Clip et l'Instant App
    url_cible = "https://nommora.com"
    qr.add_data(url_cible)
    qr.make(fit=True)
    
    # 3. Rendu aux couleurs d'Nømmora : Fond Noir Mat (#0B0F17) et Modules Or Ambré (#C8A44E)
    image_qr = qr.make_image(fill_color="#C8A44E", back_color="#0B0F17").convert('RGB')
    
    # 4. Sauvegarde directe sur le Bureau de ton Mac
